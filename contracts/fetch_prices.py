"""
fetch_prices.py — ISO New England day-ahead prices -> the chain's price feed.

Produces prices.json: for each simulated day, two vectors of 96 values, the
price a seller receives and the price a buyer pays, in currency per kWh.

  low[t]  = day-ahead LMP                      (wholesale energy, what an
                                                export is worth)
  high[t] = day-ahead LMP + delivery adder     (what a household pays: the same
                                                energy plus distribution,
                                                transmission and taxes)

The gap between them is the delivery adder. That gap is the entire reason a
local market exists: two neighbours trading directly split it instead of paying
it to the grid. The AMM prices between low and high, so both sides gain.

Coherence notes, all of which matter and none of which are obvious:

  ResStock's amy2018 release is driven by *actual* 2018 weather, so 2018 ISO-NE
  prices are contemporaneous with the loads. The heat that runs the air
  conditioning is the heat that moves the price.

  ResStock timestamps are Eastern Standard Time all year. ISO-NE publishes in
  Eastern Prevailing Time, which is daylight time in summer. That is one hour of
  offset, and getting it wrong puts the evening price peak an hour away from the
  evening load peak.

  Day-ahead LMPs are hourly. Each hour is held flat across its four quarter
  hours, which is exactly how day-ahead settlement works.

Usage:
    python fetch_prices.py --days 2018-07-01:2018-07-05 --zone WCMA
    python fetch_prices.py --days 2018-07-01:2018-07-05 --local 2018_smd_hourly.xlsx
"""
...
# ISO New England gates these downloads behind a captcha and builds the file
# list in JavaScript, so there is no URL a script can fetch. The workbook is
# downloaded once by hand and cached; everything after that is automatic.
...
def read_zone(path, zone, day_from, day_to):
    ...
    # ISO-NE labels the hour column Hr_End, not Hour, and the header has moved
    # between releases, so match on a normalised name rather than a literal one.
    ...
    # "Hour ending 1" covers 00:00-01:00, so the interval starts at hour-1.
    # ISO-NE stamps in Eastern Prevailing Time; ResStock is Eastern Standard all
    # year, so summer readings sit one hour later on the ISO clock.
...
def main():
    ...
    # Build one 96-step vector per calendar day, in ResStock's standard time.
    rows = {}
    ...
    for day in days:
        ...
        for h in range(24):
            ...
            last = v
            for _ in range(4):                      # hold the hour flat
    ...
    with open("prices_manifest.json", "w") as f:
