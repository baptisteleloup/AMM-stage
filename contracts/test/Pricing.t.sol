...
contract PricingTest is Test {
    ...
    function test_balanced() public {
        (UD60x18 r, UD60x18 c) = Pricing.prices(ud(100e18), ud(100e18), LAMBDA_LOW, LAMBDA_HIGH);
        assertApproxEqAbs(r.unwrap(), 15.16e18, TOL);
        assertApproxEqAbs(c.unwrap(), 15.16e18, TOL);
    }

    function test_surplus() public {
        (UD60x18 r, UD60x18 c) = Pricing.prices(ud(150e18), ud(100e18), LAMBDA_LOW, LAMBDA_HIGH);
        assertApproxEqAbs(r.unwrap(), 13.06e18, TOL);
        assertApproxEqAbs(c.unwrap(), 15.16e18, TOL);
    }

    function test_deficit() public {
        (UD60x18 r, UD60x18 c) = Pricing.prices(ud(100e18), ud(150e18), LAMBDA_LOW, LAMBDA_HIGH);
        assertApproxEqAbs(r.unwrap(), 15.16e18, TOL);
    ...
    function testFuzz_homogeneity(uint256 sRaw, uint256 dRaw, uint256 alphaRaw) public {
        ...
        (UD60x18 r2, UD60x18 c2) = Pricing.prices(s.mul(alpha), d.mul(alpha), LAMBDA_LOW, LAMBDA_HIGH);
    ...
    function test_explore_zeroDemand() public {
        (UD60x18 r, UD60x18 c) = Pricing.prices(ud(100e18), ud(0), LAMBDA_LOW, LAMBDA_HIGH);
    ...
    function test_edge_zeroDemand() public {
        ...
        assertEq(r.unwrap(), LAMBDA_LOW.unwrap());
        assertEq(c.unwrap(), LAMBDA_HIGH.unwrap());
    }

    function test_edge_zeroSupply() public {
        (UD60x18 r, UD60x18 c) = Pricing.prices(ud(0), ud(100e18), LAMBDA_LOW, LAMBDA_HIGH);
    ...
    function test_explore_emptyMarket() public {
        (UD60x18 r, UD60x18 c) = Pricing.prices(ud(0), ud(0), LAMBDA_LOW, LAMBDA_HIGH);
    ...
    function testFuzz_edge_extremeRatio(uint256 sRaw) public {
        ...
        assertLe(r.unwrap(), LAMBDA_HIGH.unwrap()); 
        assertGe(c.unwrap(), LAMBDA_LOW.unwrap()); 
